...
export default function EditSubsectionModal({ isOpen, onClose, onSave, subsection }: EditSubsectionModalProps) {
  const [subName, setSubName] = useState('')
  const [selectedIcon, setSelectedIcon] = useState<string>('Appliance')

  // Update form when subsection changes
  useEffect(() => {
    if (subsection) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (subName.trim()) {
      onSave(subName.trim(), selectedIcon)
      onClose()
    }
  }
  ...
  return (
    ...
    >
      <Card className="w-full max-w-sm p-6">
        <CardHeader>
          <CardTitle>Edit Subsection</CardTitle>
        </CardHeader>
